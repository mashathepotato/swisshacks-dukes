...
breakdown is shown in the client preview drawer and on the full client page.
...
$$\text{priority}=0.35\,\text{sev}+0.25\,\text{exp}+0.20\,\text{con}+0.20\,\text{rec}$$

Sub-scores:

$$\text{sev}=\frac{\text{event severity }(0\text{–}100)}{100}\qquad
\text{exp}=\frac{\text{CHF at stake}}{\max_{\text{book}}\text{ CHF at stake}}$$

$$\text{con}=w(\text{event type}),\quad w_{\text{rep}}=w_{\text{conflict}}=1,\ \ w_{\text{drift}}=w_{\text{exp}}=0.7,\ \ w_{\text{opp}}=0.3$$

$$\text{rec}=0.5^{\,\Delta t_{\text{days}}/7}\quad(\Delta t\text{ from the book's newest trigger})$$
